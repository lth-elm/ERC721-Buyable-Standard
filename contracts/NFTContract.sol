...
contract NFTContract is ERC721Buyable {
    ...
    function mint() external returns(uint) {
        ...
        attributes[supply.current()] = Attr(string(abi.encodePacked("ETH NFT #", supply.current().toString())), supply.current());
    ...
    function getSvg(uint _tokenId) private pure returns (string memory) {
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '"image_data": "', getSvg(_tokenId), '",',
                    '"attributes": [{"trait_type": "Level", "value": ', (attributes[_tokenId].level).toString(), '}',
                    ']}'
                )
            ))
        );
